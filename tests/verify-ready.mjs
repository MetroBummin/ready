...
assert.match(edgeLoginSource,/grading: item\.kind === "translation_ai"[\s\S]*kind: item\.kind === "correction_pairs" \? "correction_pairs" : "exact"/,'Assigned deterministic Workbook items must carry one explicit local grading contract');
...
assert.match(edge,/grading: item\.kind === "translation_ai"[\s\S]*mode: "deterministic"[\s\S]*answers: item\.answers/,'Deterministic Workbook answers must ship once in the assigned authenticated bundle.');
