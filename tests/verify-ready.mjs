...
import { koreanRecallCompositionState, normalizeWorkbookAnswer as normalizeWorkbookAnswerClient, livePrefixState, workbookRecallCue as workbookRecallCueClient, workbookRecallIsPendingJamo, workbookSlotCh } from '../ready/workbook-assistance.js';
...
assert.equal(workbookRecallIsPendingJamo('ㅁ','korean_syllable'),true);
assert.equal(workbookRecallIsPendingJamo('ᄆ','korean_syllable'),true);
assert.equal(workbookRecallIsPendingJamo('망','korean_syllable'),false);
assert.equal(workbookRecallIsPendingJamo('h','english_initial'),false);
assert.deepEqual(koreanRecallCompositionState('ㅅ','숙련된'),{state:'partial'});
assert.deepEqual(koreanRecallCompositionState('수','숙련된'),{state:'partial'});
assert.deepEqual(koreanRecallCompositionState('숙','숙련된'),{state:'exact'});
assert.deepEqual(koreanRecallCompositionState('사','숙련된'),{state:'mismatch'});
assert.deepEqual(koreanRecallCompositionState('ㄱ','깨끗한'),{state:'partial'});
assert.deepEqual(koreanRecallCompositionState('까','깨끗한'),{state:'mismatch'});
assert.deepEqual(koreanRecallCompositionState('고','과정'),{state:'partial'});
assert.deepEqual(koreanRecallCompositionState('과','과정'),{state:'exact'});
assert.deepEqual(koreanRecallCompositionState('달','닭고기'),{state:'partial'});
...
assert.doesNotMatch(studentApp,/function flushWorkbookAttempts[^\n]*renderWorkbook\(\)/,'Background persistence must never redraw or blur the active Workbook item');
...
assert.match(app,/verbForm&&slot===0\?' verb-form-leading-slot'/,'Only the first Stage 5 base-verb placeholder may retain its focused hint');
assert.match(read('ready/design.css'),/\.workbook-blank\.verb-form-leading-slot input:focus::placeholder\{color:var\(--ready-primary\)/,'The focused first Stage 5 blank must keep its base-verb cue visible');
assert.doesNotMatch(read('ready/design.css'),/\.workbook-blank\.verb-form-slot input:focus::placeholder/,'Later Stage 5 slots must use the normal focused placeholder behavior');
...
assert.match(app,/function syncWorkbookSlotWidth\(input\)[^\n]*workbookFixedWidth==='true'/,'Fixed inline blanks must not reflow while typing');
assert.match(read('ready/design.css'),/width:calc\(var\(--workbook-slot-ch,8\)\*1ch \+ 18px\);min-width:46px/,'Inline blanks must keep compact answer-sized horizontal chrome');
assert.match(app,/workbook-prompt inline-blank-task/,'Only inline blank prompts must opt into extra mobile line spacing');
assert.match(read('ready/design.css'),/@media\(max-width:480px\)\{[\s\S]*\.workbook-prompt\.inline-blank-task\{line-height:2\.05\}[\s\S]*\.workbook-blank input\{min-height:37px;height:37px;padding:4px 7px;border-radius:10px\}/,'Phone inline blanks must be compact with enough multi-line breathing room');
assert.match(app,/progressive\.visible\.map\(chipIndex=>\{const used=consumedSet\.has\(chipIndex\),wrong=wrongChip===chipIndex;return `<span class="workbook-order-bank-slot/,'Stage 8 must render only the current fixed answer-position batch in its stored shuffled order');
...
assert.match(app,/aria-pressed="\$\{used\}" aria-invalid="\$\{wrong\?'true':'false'\}" \$\{used\|\|result\?'disabled':''\}/,'Selected Stage 8 chips must become unavailable without leaving their slots');
assert.doesNotMatch(read('ready/design.css'),/workbook-order-bank-slot\.used\{[^}]*display:none/,'Selected Stage 8 chip slots must remain in the original layout');
assert.match(read('ready/design.css'),/workbook-order-bank-slot\.used button\{[^}]*opacity:\.42/,'Selected Stage 8 chips must remain visible with a subdued used state');
assert.match(read('ready/design.css'),/workbook-order-bank-slot\.wrong button\{[^}]*border-color:var\(--ready-error\)/,'A practice mistake must turn only its still-available chip red');
...
assert.match(read('ready/design.css'),/workbook-order-bank\{[^}]*min-height:var\(--workbook-order-bank-height/,'Stage 8 must preserve its measured bank height across local refreshes');
assert.match(app,/workbookOrderClick\(words,answer,current,consumed\[group\],chipIndex,\{practice\}\)/,'Stage 8 must pass the per-problem practice snapshot into chip grading');
assert.match(app,/click\.type==='practice-wrong'\)return flashWorkbookOrderWrongChip\(session,item,group,click\.chipIndex\)/,'Practice Stage 8 mistakes must stay local and retryable');
assert.match(app,/if\(click\.type==='wrong'\)return submitWorkbook\(\)/,'Real Stage 8 mistakes must still grade immediately');
...
assert.doesNotMatch(edge,/grading:[^\n]*server_deterministic/,'Deterministic Workbook grading must not require a per-submit round trip.');
...
assert.doesNotMatch(app,/WORKBOOK_KOREAN_RECALL_STABILIZE_MS/,'Korean recall correctness must not depend on elapsed time');
assert.match(app,/koreanRecallCompositionState\(raw,item\.grading\?\.answers\?\.\[index\]\)/,'Korean recall must compare the visible syllable by Hangul composition state');
