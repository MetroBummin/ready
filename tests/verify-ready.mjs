...
assert.match(app,/progressive\.visible\.map\(chipIndex=>\{const used=consumedSet\.has\(chipIndex\);return `<span class="workbook-order-bank-slot/,'Stage 8 must render only the current fixed answer-position batch in its stored shuffled order');
...
assert.match(app,/aria-pressed="\$\{used\}" \$\{used\|\|result\?'disabled':''\}/,'Selected Stage 8 chips must become unavailable without leaving their slots');
