...
let result=await callContext('studio_author');context.studio=result.studio;assert.equal(aiCalls.length,1);assert.equal(aiCalls[0].length,9);
...
await pg.query('insert into ready_exam_passages(exam_id,passage_id,position) values($1,$2,0)',[exam,passageId]);
...
result=await callContext('studio_author');context.studio=result.studio;assert.deepEqual(aiCalls[1],[edited[3].id]);
...
assert.equal((await pg.query('select * from ready_workbook_catalogs')).rows.length,1);assert.equal(aiCalls.length,2);
...
await assert.rejects(()=>call('studio_split_draft',mergeBody,admin));assert.equal(aiCalls.length,2);
