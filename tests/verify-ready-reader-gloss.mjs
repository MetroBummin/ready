...
assert.match(glossModule,/sourceBox=\{width:Math\.ceil\(sourceRect\.width\+8\),height:Math\.ceil\(sourceRect\.height\)\}/,'Reader lookup must preserve the waiting box dimensions for shorter glosses');
assert.match(glossModule,/--reader-source-width:\$\{minWidth\}px[\s\S]*--reader-source-height:\$\{minHeight\}px/,'Resolved gloss markup must expose the measured source dimensions');
assert.match(css,/reader-inline-source\.is-pending::before,[^}]*reader-inline-source\.is-refreshing::before[^}]*background:color-mix\([^}]*ready-primary[^}]*opacity:1/,'Reader pending feedback must fill an expanded rectangular token box without shifting prose');
...
assert.match(css,/reader-inline-replacement\{[\s\S]*min-width:var\(--reader-source-width,0px\)[\s\S]*min-height:var\(--reader-source-height,1\.55em\)[\s\S]*border-radius:var\(--ready-radius-sm\)[\s\S]*background:color-mix/,'Resolved gloss must retain source dimensions inside the agreed tinted box');
assert.match(css,/body\[data-ready-mode\] \.reader-inline-gloss\{[^}]*color:var\(--ready-ink\)[^}]*background:transparent[^}]*font:520 \.94em\/1\.35/,'Korean gloss must remain optically aligned with adjacent English');
assert.match(css,/reader-word-toggle\{[^}]*width:1\.58em[^}]*border-left:[^}]*color:var\(--ready-primary\)[^}]*font:820 \.78em\/1/,'Save and remove marks must live inside a visible fixed-width control segment');
