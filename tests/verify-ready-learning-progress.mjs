...
assert.match(adminApp,/Workbook 상태[\s\S]*현재 오답/,'student overview must keep current Workbook status and unresolved wrong answers without inventing a combined percent');
assert.match(adminApp,/지문별 Workbook[\s\S]*passageProgressHtml/,'student detail must drill down by passage and workbook stage');
assert.match(adminApp,/stage\.cumulativeProgress[\s\S]*stage\.correctClears[\s\S]*stage\.total/,'Admin detail must render the stage-level cumulative progress contract');
assert.doesNotMatch(adminApp,/stage\.progress|passage\.progress|workbook\.progress/,'Admin UI must not use the deprecated aggregate or stage progress fields');
...
assert.match(server,/periodAttempts = currentAttempts\.filter/,'Admin date filters must apply to activity metrics, not cumulative stage progress');
