...
async function factoryExercises(rows: any[], requestedTargets: Record<number, number[]>) {
  ...
  for (const stage of stages) for (let offset = 0; offset < targets[stage].length && !quotaStopped; offset += 6) {
    ...
    const examples = stage === 6 ? 'Example: canonical "She enjoys music." -> {"sentenceIndex":1,"prompt":"She ____________ music.","wrong":"enjoy","answer":"enjoys"}.' : stage === 7 ? 'Example: canonical "She enjoys music." -> {"sentenceIndex":1,"sentence":"She enjoy music.","wrong":"enjoy","correct":"enjoys"}.' : '';
    const prompt = `Create only source-grounded READY Stage ${stage} exercises for the supplied canonical sentence pairs. Return exactly {"${stage}":[...]} and no other key. Item fields: ${fields}. Eligible sentence indexes: ${JSON.stringify(batch)}. Return exactly one item for every eligible sentence index and preserve each canonical sentence byte-for-byte except for the one requested blank or substitution. The wrong value is an intentional student-facing distractor, not a change to the canonical answer. For Stage 7, if a natural grammar error is difficult, use one clear lexical or inflection error whose correction restores the canonical sentence exactly. ${examples} Never create a new canonical answer.\n${JSON.stringify(source)}`;
    callCount += 1;
    try {
      const result = await factoryGemini(prompt, Math.min(8_192, 220 * batch.length + 500)); tokenUsage += result.tokenUsage;
      const accepted = (Array.isArray(result.parsed?.[stage]) ? result.parsed[stage] : []).filter((item: any) => batch.includes(Number(item?.sentenceIndex)));
