...
async function callGeminiInlineGloss(context: ReaderGlossPromptContext) {
  ...
  for(const generationConfig of [{...base,thinkingConfig:{thinkingBudget:0}},base]){
    const response=await fetch(url,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({system_instruction:{parts:[{text:GEMINI_SYSTEM}]},contents:[{role:"user",parts:[{text:geminiInlineGlossPrompt(context)}]}],generationConfig})});
...
async function geminiSentenceJson(prompt:string,maxOutputTokens=500){const provider=(Deno.env.get("AI_PROVIDER")??"").trim().toLowerCase(),key=Deno.env.get("GEMINI_API_KEY");if(provider!=="gemini"||!key)throw new ApiError(503,"Gemini 문장 학습 기능이 아직 연결되지 않았습니다.");const url=`https://generativelanguage.googleapis.com/v1beta/models/${GEMINI_MODEL}:generateContent?key=${key}`,response=await fetch(url,{method:"POST",headers:{"content-type":"application/json"},body:JSON.stringify({system_instruction:{parts:[{text:GEMINI_SYSTEM}]},contents:[{role:"user",parts:[{text:prompt}]}],generationConfig:{maxOutputTokens,temperature:0.1,responseMimeType:"application/json",thinkingConfig:{thinkingBudget:0}}})});if(!response.ok)throw new ApiError(502,"Gemini 문장 학습 결과를 받을 수 없습니다.");const payload=await response.json(),parsed=parseJson((payload?.candidates?.[0]?.content?.parts||[]).map((part:any)=>part?.text||"").join(""));if(!parsed)throw new ApiError(502,"Gemini 문장 학습 결과 형식이 올바르지 않습니다.");return parsed;}
