...
- Reader와 Question passage는 연속된 plain prose입니다. `READER_INLINE_GLOSS_ENABLED` 실험이
  켜진 경우 Reader에서만 단어를 선택해 모바일 bottom sheet 또는 넓은 화면 right panel에서
  사전/AI 문맥 뜻 draft를 편집할 수 있습니다. 본문 영어는 뜻으로 치환하지 않습니다.
  Question/Workbook/Review에는 새 lookup 이벤트가 연결되지 않습니다. 기존 운영 데이터와 관련
  테이블은 삭제하지 않습니다.
...
- 기존 lexical 데이터는 보존합니다. 단어 lookup operation은 Reader surface만 허용합니다.
