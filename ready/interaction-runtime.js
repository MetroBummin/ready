...
export function contractPassageHtml(contract,{escape,result=null,inlineSelected=[],selected=[]}={}){
  if(!contract?.passage?.visible)return '';
  const segments=list(contract?.passage?.segments),html=segments.map(segment=>{
    if(segment.kind==='text')return escape(segment.text);
    if(segment.kind==='annotation')return `<span class="passage-pointer" data-contract-device="${escape(segment.id)}">${escape(segment.label)}${escape(segment.text)}</span>`;
...
export function contractChoiceCopyHtml(contract,index,escape){
  const cells=list(contract?.choices?.rows?.[index]?.cells);
  if(!cells.length)return '';
  if(contract.kind!=='choice_matrix')return escape(cells[0]);
  return cells.map((cell,column)=>`<span class="choice-cell" data-choice-column="${column}"><small>${escape(contract.choices.columns[column]||'')}</small>${escape(cell)}</span>`).join('');
}
