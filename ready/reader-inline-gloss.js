...
function sourceMarkup(slice,sentence,pendingByStart,esc,_limit,base=0){
  let cursor=0,html='';for(const token of readerSentenceTokens(slice)){const start=base+token.start,end=base+token.end,pending=pendingByStart.get(start);html+=esc(slice.slice(cursor,token.start));html+=`<button type="button" class="reader-inline-source${pending?.loading?' is-loading':''}${pending?.error?' is-error':''}" data-reader-gloss-source data-sentence-id="${esc(sentence.id)}" data-start="${start}" data-end="${end}" aria-busy="${pending?.loading?'true':'false'}" aria-label="${esc(token.text)} 뜻 보기">${esc(token.text)}</button>`;cursor=token.end;}return html+esc(slice.slice(cursor));
}
...
export function createReaderInlineGloss({root,passage,sentences,request,reducedMotion=globalThis.matchMedia?.('(prefers-reduced-motion: reduce)')?.matches===true}){
  ...
  function activate(button){
    ...
    const cached=readReaderGlossCache(key);if(cached){apply(cached,sentenceId,intentKey,next,anchorY);return;}
    const status={sentenceId,start,end,loading:false,error:false};pending.set(intentKey,status);const timer=setTimeout(()=>{if(pending.get(intentKey)===status){status.loading=true;redraw(sentenceId);}},READER_GLOSS_LOADING_DELAY_MS);
    resolveReaderGlossCached(key,()=>request({passageId:passage.id,passageRevision:revision,sentenceId,start,end,sourceText})).then(result=>{clearTimeout(timer);pending.delete(intentKey);if(destroyed||intent.get(intentKey)!==next||passage.id!==root.dataset.readerPassageId)return;if(validReaderGlossResult(result,{sentenceId,text:sentence.text}))apply(result,sentenceId,intentKey,next,anchorY);else redraw(sentenceId);}).catch(()=>{clearTimeout(timer);pending.delete(intentKey);if(destroyed||intent.get(intentKey)!==next)return;status.error=true;pending.set(intentKey,status);redraw(sentenceId);setTimeout(()=>{if(pending.get(intentKey)===status){pending.delete(intentKey);redraw(sentenceId);}},1200);});
  }
  function apply(result,sentenceId,intentKey,expected,anchorY){if(destroyed||intent.get(intentKey)!==expected)return;const node=sentenceNode(sentenceId),list=(replacements.get(sentenceId)||[]).filter(item=>!rangesOverlap(item,result));list.push(result);replacements.set(sentenceId,list);redraw(sentenceId,{y:anchorY,selector:`[data-reader-range="${result.start}:${result.end}"]`,animate:true});node?.querySelector(`[data-reader-gloss-reset="${result.start}:${result.end}"]`)?.focus({preventScroll:true});root.querySelector('[data-reader-gloss-live]').textContent=`${result.sourceText}, ${result.gloss}`;}
  ...
  function onClick(event){const resetButton=event.target.closest?.('[data-reader-gloss-reset]');if(resetButton){event.preventDefault();reset(resetButton);return;}if(event.detail===0){const source=event.target.closest?.('[data-reader-gloss-source]');if(source)activate(source);}}
  ...
  return {destroy(){destroyed=true;root.removeEventListener('pointerdown',onPointerDown);root.removeEventListener('pointerup',onPointerUp);root.removeEventListener('pointercancel',onPointerCancel);root.removeEventListener('click',onClick);pending.clear();replacements.clear();}};
}
