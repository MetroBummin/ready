-- Sentence learning is deterministic: source text + teacher translation only.
-- Keep ready_sentence_bakes and saved_sentences.analysis_snapshot dormant for
-- production compatibility, but lexical rebakes no longer read or write them.
...
begin
  if jsonb_typeof(p_bake->'sentences') <> 'array' then raise exception 'Lexical bake sentences array is required.'; end if;
  ...
  for s in select * from jsonb_array_elements(p_bake->'sentences') loop
    if not exists(select 1 from ready_passage_sentences where id=(s->>'sentenceId')::uuid and passage_id=p_passage_id) then raise exception 'Lexical bake contains a foreign sentence.'; end if;
    ...
    for c in select * from jsonb_array_elements(coalesce(s->'concepts','[]')) loop
      ...
      if cardinality(v_token_ids) <> jsonb_array_length(c->'tokenIndexes') then raise exception 'Lexical bake token mapping is invalid.'; end if;
