-- Preserve lexical identity across rebakes and model/prompt changes.
create table if not exists public.ready_lexical_concept_aliases (
  alias_key text primary key,
  concept_id uuid not null references public.ready_lexical_concepts(id) on delete cascade,
  created_at timestamptz not null default now()
);
insert into public.ready_lexical_concept_aliases(alias_key,concept_id)
select concept_key,id from public.ready_lexical_concepts on conflict(alias_key) do nothing;
...
alter table public.ready_lexical_concept_aliases enable row level security;
revoke all on public.ready_lexical_concept_aliases from anon,authenticated;
grant all on public.ready_lexical_concept_aliases to service_role;
